...
namespace Modbus_Connect
{
    ...
    public class Connect
    {
        ...
        public static string FunctionCode0x0F(int nSlaveID, string DevAddr, int nDevCount, int value)
        {
            ...
            if (value == 1)
            {
                int binaryValue = (1 << nDevCount) - 1;  // 先生成 n 個 1，再左移

                // 轉換為十六進制
                text3 = binaryValue.ToString("X");
            }
            else
            {
                text3 = "00";
            }
            return text + "0F" + DevAddr + text2 + "01" + text3;
        }
        public static string FunctionCode0x0F(int nSlaveID, string DevAddr, int nDevCount, string longvalue)
        {
            string text3 = "";
            string text = Operation.ToHex(nSlaveID, bWord: false);
            DevAddr = Operation.Right("0000" + DevAddr, 4);
            string text2 = Operation.ToHex(nDevCount);
            //11110000 11111000 11111000 
            // 若不是8的倍數，右邊補0

            int remainder = longvalue.Length % 8;
            int byteCount = (int)Math.Ceiling(nDevCount / 8.0);
            string byteCountHex = byteCount.ToString("X2");
            if (remainder != 0)
            {
                longvalue = longvalue.PadRight(longvalue.Length + (8 - remainder), '0');
            }

            // 整體反轉字串（低位優先）
            char[] reversed = longvalue.ToCharArray();
            //Array.Reverse(reversed);
            string reversedBinary = new string(reversed);

            // 每8位切割並轉成16進位
            List<string> hexParts = new List<string>();
            for (int i = 0; i < reversedBinary.Length; i += 8)
            {
                string byteStr = reversedBinary.Substring(i, 8);

                char[] byteChars = byteStr.ToCharArray();

                string correctedByte = new string(byteChars);

                int value = Convert.ToInt32(correctedByte, 2);
                hexParts.Add(value.ToString("X2"));
            }

            string result = string.Concat(hexParts);
            Console.WriteLine(result);  // 👉 輸出：1F 0F 0F
            Console.WriteLine(text + "0F" + DevAddr + text2 + byteCountHex + result);
            return text + "0F" + DevAddr + text2 + byteCountHex + result;
        } 
        ...
        public static bool DoFunctionCode0x0F(string name, string DevAddr, int nDevCount,string longvalue, out SENDRECEIVE_INFO SRInfo)
        {
            SRInfo = default(SENDRECEIVE_INFO);
            ...
            {
                // 生成 PDU 字串
                string sendStringPDU = FunctionCode0x0F(GlobalVar.SRCond.nSlaveID, DevAddr, nDevCount,longvalue);
